                        ...
                        <div class="mm-survey">
                            ...
                            <div class="mm-survey-results">
                                ...
                                <div class="mm-survey-results-controller">
                                    ...
                                    <div class="mm-quizInv-btn" style="display: inline-block;position: relative;float: right;margin: 12px 15px;">
                                        <button class="bubbly-button" style="padding: 3px 40px;background-color: #00adb5;box-shadow: 0 2px 25px rgba(0, 173, 181, 0.5);"
                                                onclick="window.open('{{route('dashboard')}}')">Dashboard</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mm-survey-bottom">
                                <div class="mm-survey-container">
                                    <div class="status-timer" style="animation-duration:1s">
                                        <i class="fa fa-stopwatch"></i><span class="time-remaining" style="margin-left: 5px"></span>
                                    </div>
                                    @foreach($data_soal as $row)
                                        <div class="mm-survey-page" data-page="{{$no++}}">
                                            <div class="mm-survey-content">
                                                <div class="mm-survey-question">{{$row->soal}}</div>
                                                <div class="mm-survey-item">
                                                    <input type="radio" id="op-a{{$row->id}}" name="" data-item=""
                                                           class="op{{$row->id}}" value="A">
                                                    <label for="op-a{{$row->id}}"><span></span>
                                                        <p>{{$row->pilihan_a}}</p></label>
                                                </div>
                                                <div class="mm-survey-item">
                                                    <input type="radio" id="op-b{{$row->id}}" name="" data-item=""
                                                           class="op{{$row->id}}" value="B">
                                                    <label for="op-b{{$row->id}}"><span></span>
                                                        <p>{{$row->pilihan_b}}</p></label>
                                                </div>
                                                <div class="mm-survey-item">
                                                    <input type="radio" id="op-c{{$row->id}}" name="" data-item=""
                                                           class="op{{$row->id}}" value="C">
                                                    <label for="op-c{{$row->id}}"><span></span>
                                                        <p>{{$row->pilihan_c}}</p></label>
                                                </div>
                                                <div class="mm-survey-item">
                                                    <input type="radio" id="op-d{{$row->id}}" name="" data-item=""
                                                           class="op{{$row->id}}" value="D">
                                                    <label for="op-d{{$row->id}}"><span></span>
                                                        <p>{{$row->pilihan_d}}</p></label>
                                                </div>
                                                <div class="mm-survey-item">
                                                    <input type="radio" id="op-e{{$row->id}}" name="" data-item=""
                                                           class="op{{$row->id}}" value="E">
                                                    <label for="op-e{{$row->id}}"><span></span>
                                                        <p>{{$row->pilihan_e}}</p></label>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <div class="mm-survey-controller">
                                    ...
                                    <div class="mm-finish-btn">
                                        <form method="post" id="form-submit-quiz">
                                            {{csrf_field()}}
                                            <input type="hidden" name="id_siswa" value="{{Auth::id()}}">
                                            <input type="hidden" name="token" value="{{$token->token}}">
                                            <input type="hidden" name="id_mapel" value="{{$token->id_mapel}}">
                                            <input type="hidden" name="nilai" id="quiz_score">
...
@push("scripts")
    <script>
        ...
        function init() {
            ...
            @php $x = 1; $y = 1; @endphp
            @foreach($data_soal as $row)
            $('.op{{$row->id}}').attr('name', 'op{{$x++}}').attr('data-item', '{{$y++}}');
            @endforeach

            $('.mm-page-1').addClass('active');

            quiz_time_limit = '{{$token->durasi}}';
            ...
            $('.mm-next-btn').on('click', function () {
                ...
                if ($('.mm-page-' + count).hasClass('active')) {
                    if ($('.mm-page-' + count).hasClass('pass')) {
                        ...
                        $('.mm-page-' + count + ' .mm-survey-content .mm-survey-item').on('click', function () {
                            $('.mm-finish-btn').addClass('active');
                        });
                    }
                } else {
        ...
        function getCurrentSlide() {
            $('.mm-survey-page').each(function () {
                ...
                $('.mm-progress-bar-text').text(results == undefined ? 'Question ' + x + ' of {{count($data_soal)}}' :
                    'Results Review: Question ' + x + ' of {{count($data_soal)}}');
                return x;
            });
        }
        ...
        function checkStatus() {
            $('.mm-survey-content .mm-survey-item').on('click', function () {
                var item;
                item = $(this);
        ...
        function collectData() {
            $.ajax({
                url: '{{route('load.quiz.answers', ['ids' => implode(',',$data_soal->pluck('id')->toArray())])}}',
                type: "GET",
                beforeSend: function () {
                    $('#image').show();
                ...
                success: function (data) {
                    ...
                    for (i = 1; i <= count; i++) {
                        ...
                        if (map[i] == undefined) {
                            ...
                            if (map[i] == ax[i - 1]) {
                                answer_status = 'Jawaban yang tepat:';
                                g = map[i];
                                p = 'benar';
                                c = 1;
                            } else {
        ...
        function submitQuiz() {
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        console.log('Quiz #{{$token->token}} is successfully submitted!');
                    }
                },
                error: function () {
                    swal({
